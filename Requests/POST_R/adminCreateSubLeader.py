...
def create_sub_leader(ns):
    #Post request for creating a new leader user & belonging database
    @ns.route('/createSubLeader')
    class CreateLeaderUser(Resource):
        new_sub_user_model = UM.sub_leader_model(ns)
        @ns.doc('admin_create_sub_leader_user',
                description='Create new leader user when given Email, Password and Account type.',
                responses={
                    200: 'OK',
